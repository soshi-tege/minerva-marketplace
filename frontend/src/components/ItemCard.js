...
export default function ItemCard({ item }) {
  ...
  return (
    <Link to={`/items/${item.id}`} style={{ textDecoration: "none", color: "inherit", width: "100%" }}>
      <div
        className="card item-card"
        style={{ cursor: "pointer", transition: "transform 0.15s", display: "flex", flexDirection: "column", gap: "6px", width: "100%" }}
        onMouseEnter={e => e.currentTarget.style.transform = "translateY(-3px)"}
        onMouseLeave={e => e.currentTarget.style.transform = "translateY(0)"}
      >
        <img src={PLACEHOLDER} alt={item.title} style={{ width: "100%", objectFit: "cover", borderRadius: "8px" }} />
