...
export default function Items() {
  ...
  if (!items.length) return <p>No items yet. Be the first to post something!</p>;
  return (
    <div>
      <h2>Browse items</h2>
      <div className="grid">
        {items.map((item) => (
          ...
          >
            ...
            <h3>{item.title}</h3>
            {item.price != null && <p style={{ fontWeight: 600 }}>${item.price}</p>}
            {item.campus && <p style={{ fontSize: 14, color: "#666" }}>{item.campus}</p>}
          </Link>
        ))}
      </div>
    </div>
