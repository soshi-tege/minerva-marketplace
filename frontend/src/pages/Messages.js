...
export default function Messages() {
  ...
  const storedUser = JSON.parse(localStorage.getItem("mm_auth_user") || "{}"); const currentUserId = storedUser?.id;
    ...
    <div className="container messages" style={{ display: "flex", gap: "1rem", padding: "1rem" }}>
      <div className="card conversations" style={{ width: "250px", padding: "1rem" }}>
        ...
        {conversations.map((c) => (
          <div
            ...
            style={{ cursor: "pointer", padding: "0.5rem", fontWeight: selectedConvo?.id === c.id ? "bold" : "normal" }}
          >
      ...
      <div className="card chat" style={{ flex: 1, padding: "1rem", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
        ...
        {messages.map((m) => (
          <div
            key={m.id}
            className={`message ${m.sender_id === currentUserId ? "you" : ""}`}
            style={{ textAlign: m.sender_id === currentUserId ? "right" : "left" }}
          >
            {m.body}
          </div>
        ))}
